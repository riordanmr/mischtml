...
function convertToHTML(line)
    local html
    html = line:gsub('&','&amp;')
    -- Enable this to render my comments in italics.
    if false then
        html = html:gsub('%[','<em>[')
        html = html:gsub(']',']</em>')
    end
...
function startLetter(curDate)
    ...
    outLine('<html>')
    outLine('<head>')
    outLine('  <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />')
    ...
    outLine('  <table width="100%" class="navbar">')
    outLine('    <tr>')
...
function main()
    ...
    for line in filein:lines() do
        ...
        elseif inLetters then
            ...
            --print(html)
            isDearLine = line:starts("Dear") or line :starts("P. S. (To last nite's letter)")
            if isDearLine then
